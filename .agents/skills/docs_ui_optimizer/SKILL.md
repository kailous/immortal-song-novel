...
2.  **验证结果**: 检查 `/docs/chapters` 目录下是否生成了对应的 JSON 文件，并确认 `index.json` 已更新。
3.  **上传静态资源**: 如有新增插图，放入 `/docs/images` 并在 JSON 中引用。

### 2.2 更新图鉴详情
1.  **执行同步脚本**: 在根目录下运行 `python3 .agents/skills/docs_ui_optimizer/scripts/sync_characters.py`。
2.  **验证结果**: 检查 `/docs/data/characters.json` 是否已更新。
3.  **头像处理**: 如果新增了角色，请确保将对应头像放入 `.images/图鉴/人物` 或 `道具` 目录下，并确保脚本已正确将其同步（或手动复制）到 `/docs/images`。
