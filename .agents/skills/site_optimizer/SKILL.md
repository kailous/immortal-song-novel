...
- 词典公开源现在也在 `docs/content/glossary/`，其索引层在 `docs/data/glossary_zh.json` 与 `docs/data/glossary_en.json`。
- 发布图片按用途分层：
  - `docs/images/characters/`：角色、道具、势力
  - `docs/images/branding/`：logo、分享封面、品牌资产
  - `docs/images/chapters/`：按章节存放插图与海报

## 常用流程
1. 同步内容：`make publish`、按需补跑 `make sync-chars` 与 `make sync-glossary`。
2. 压缩资源：`make optimize-assets`。
   - 默认把 `docs/images` 中的发布内容图转成 `webp`，更新 `docs/` 内所有发布引用，并删除已替换的原图。
   - `branding/og-cover.jpg` 保留原格式，用于社交分享兼容。
3. 若是章节海报，先执行 `make compose-poster SRC=... OUT=...`，用 `资源/品牌/poster-template-overlay.png` 叠加统一版式。
4. 同步页面元信息：`make sync-head`。
5. 发布审计：`make audit`。
6. 若审计失败，先修复硬错误，再重新运行审计。

## 脚本
- `scripts/audit_site.py`：校验 JSON、JS 语法、外部资源、图片体积、角色图片引用、章节索引和词典索引。
...
- `../illustration_generator/scripts/apply_poster_template.py`：将 `资源/品牌/poster-template-overlay.png` 覆盖到章节插图上，统一成片版式。
...
- 所有 `docs/data/glossary_*.json` 必须能回指到真实存在的 Markdown 词典源。

## 推荐发布顺序
1. `make publish`
2. `make sync-chars`
3. `make sync-glossary`
4. 需要时 `make compose-poster SRC=... OUT=...`
5. `make optimize-assets`
6. `make sync-head`
7. `make audit`
8. `git commit`
9. `git push`
